...
class dk_speakout_SpeakOut
{
	 ...
	 *
	 * @param string $tweet the x message
	 * @return string: a properly url-encoded tweet
	 */
	public static function x_encode( $tweet ) {
	...
	//	$pager_html  .= '<input type="hidden" name="dk-speakout-page" id="dk-speakout-page" value="' . $page_handle . '">';
		if($total_pages > 1){
		      $pager_html  .= '<span class="displaying-num">' . $count . ' ' . __( 'items', 'speakout' ) . ' </span>';
		}
		else{
		       $pager_html  .= '<span class="displaying-num">' . __( 'page', 'speakout' ) . ' </span>';   
		}
		...
		// #page of #pages text
			$pager_html .= '<span class="paging-input"> ' . $current_page . ' ' . __( 'of', 'speakout' ) . ' <span class="total-pages">' . $total_pages . ' </span></span>';
